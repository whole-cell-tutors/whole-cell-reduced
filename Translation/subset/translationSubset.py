...
##############
## Creates subset of Translation module
...
Translation_Species_List_Subset= [line.split(',') for line in FileWIthSubset.readlines()]
Translation_Species_List_Subset=Translation_Species_List_Subset[0]
FileWIthSubset.close()
Translation_Species_List_Subset=[x.strip() for x in Translation_Species_List_Subset]
###
Translation_Reactions_List_Subset=map(lambda x: "Translation_"+x[:-3], Translation_Species_List_Subset)
...
All_reactants_list_tmp=[x[1] for x in All_reactants_list]

# do not include monomer name
Products_list=Get_Field(Translation_Reactions_Dict, speciesIDFortRNAlist, 'products')[1:]
Products_list_tmp=[x[1] for x in Products_list]

TUs_list=[]
for reactionID in Translation_Reactions_List_Subset:
    TU=Get_Field(Translation_Reactions_Dict, reactionID, 'enzymes')[0]
    TUs_list.append(TU)

#unique TUs
TUs_list=list(set(TUs_list))
    
Enzymes_list=Get_Field(Translation_Reactions_Dict, reactionID, 'enzymes')[1:]


Translation_Species_List_Subset=Translation_Species_List_Subset+All_reactants_list_tmp+Products_list_tmp+TUs_list+Enzymes_list

Translation_Species_Dict_tmp={k:Translation_Species_Dict[k] for k in Translation_Species_List_Subset}
Translation_Reactions_Dict_tmp={k:Translation_Reactions_Dict[k] for k in Translation_Reactions_List_Subset}


#TODO remove
Translation_Species_Dict=Translation_Species_Dict_tmp
Translation_Reactions_Dict=Translation_Reactions_Dict_tmp
...
def create_model(Translation_Species_List,Translation_ReactionID_List):
    ...
    return writeSBMLToFile(document,'TranslationSubset.xml')
